						...
						<table class="table-subj" width="100%">
							...
							foreach($classes as $ind => $class) { 
								...
								if($c == 1){									
									...
									$tr1 .= '<td'.$align.' data-cl="'.$cur_class.'" data-teach="'.$tmp['tname'].'" class="'.$flags_cl.'" valign=top colspan='.$colspan.'>'.$tmp['sname'].($flag_text!='' ? '<span class="span-flags">'.$flag_text.'</span>':'').'</td>';
